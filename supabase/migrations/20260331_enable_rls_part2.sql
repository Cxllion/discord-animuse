...
create policy "Enable all access for service role on bingo_buffer" on public.bingo_buffer for all using (true) with check (true);
create policy "Enable all access for service role on subscriptions" on public.subscriptions for all using (true) with check (true);
create policy "Enable all access for service role on tracked_anime_state" on public.tracked_anime_state for all using (true) with check (true);
